...
export class CloneableRepositoryFilterList extends React.PureComponent<ICloneableRepositoryFilterListProps> {
  ...
  public render() {
    ...
    return (
      <SectionFilterList<ICloneableRepositoryListItem>
        ...
        placeholderText={t('clone.filterPlaceholder')}
  ...
  private getYourRepositoriesLabel = () => {
    return t('clone.yourRepositories')
  }
  ...
  private renderItem = (
    ...
    return (
      <div className="clone-repository-list-item">
        ...
        {item.archived && <div className="archived">{t('clone.archived')}</div>}
      </div>
    )
  }

  private renderPostFilter = () => {
    const tooltip = t('clone.refreshRepositories')
  ...
  private renderNoItems = () => {
    ...
    if (this.props.filterText.length !== 0) {
      return (
        <div className="no-items no-results-found">
          <div>
            {t('clone.noMatchingRepository')} <Ref>{this.props.filterText}</Ref>
          </div>
        </div>
      )
    }
    ...
    return (
      <div className="no-items empty-repository-list">
        <div>
          {t('clone.emptyRepositoryListPrefix')}{' '}
          <Ref>{this.props.account.login}</Ref>{' '}
          {t('clone.emptyRepositoryListMiddle', {
            endpoint: account.friendlyEndpoint,
          })}{' '}
          <LinkButton onClick={this.refreshRepositories}>
            {t('clone.refreshThisList')}
          </LinkButton>
          {t('clone.emptyRepositoryListSuffix')}
        </div>
      </div>
    )
  }
