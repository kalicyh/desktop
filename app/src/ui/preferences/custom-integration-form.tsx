...
> {
    ...
    return (
      <div className="custom-integration-form-container">
        <div className="custom-integration-form-path-container">
          <TextBox
            label={t('preferences.integrations.custom.path.label')}
            value={this.state.path}
            ref={this.pathInputRef}
            onValueChanged={this.onPathChanged}
            placeholder={t('preferences.integrations.custom.path.placeholder')}
            ariaDescribedBy={`${this.props.id}-custom-integration-path-error`}
          />
          <Button onClick={this.onChoosePath}>
            {t('preferences.integrations.custom.choosePath')}
          </Button>
        </div>
        {this.renderPathErrors()}
        <TextBox
          label={t('preferences.integrations.custom.arguments.label')}
  ...
  private renderArgsErrors() {
    ...
    const errorDescription = this.state.showNonValidArgsError
      ...
      : t('preferences.integrations.custom.missingTargetPathArgument', {
  ...
  private onChoosePath = async () => {
    ...
    const path = await showOpenDialog({
      ...
      filters: __WIN32__
        ? [
            {
              name: t('preferences.integrations.custom.executableFilter'),
